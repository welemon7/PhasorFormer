...
def init_distributed_mode():
    ...
    elif hasattr(opt, 'local_rank') and opt.local_rank is not None:
...
print(dir_name)
opt = options.Options().init(argparse.ArgumentParser(description='mask removal')).parse_args()
if not hasattr(opt, 'local_rank'):
    opt.local_rank = 0
print(opt)
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"

local_rank, world_size = init_distributed_mode()
rank = local_rank


assert opt.batch_size % world_size == 0, '3Q2742'
...
# ---------------------------- DDP  ---------------------------- #
model_restoration = model_restoration.to(local_rank)
model_restoration = torch.nn.parallel.DistributedDataParallel(
    model_restoration, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False
)
...
if rank == 0:
    print(f"Size of training set: {len_trainset}, validation set: {len_valset}")
...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in train_pbar:
        ...
        mask = data[2].cuda(local_rank, non_blocking=True)
        ...
        # ---------------------------- Evaluation---------------------------- #
        if rank == 0 and (global_step % eval_now == 0):
            ...
            with torch.no_grad():
                ...
                for ii, data_val in val_pbar:
                    target = data_val[0].cuda(local_rank, non_blocking=True)
                    input_ = data_val[1].cuda(local_rank, non_blocking=True)
                    mask = data_val[2].cuda(local_rank, non_blocking=True)
...
# ---------------------------- test ---------------------------- #
if epoch == opt.nepoch and rank == 0:
    # coding=gb2312
    import numpy as np
    import os, sys
    import argparse
    ...
    parser.add_argument('--win_size', type=int, default=12, help='number of data loading workers')
    ...
    with torch.no_grad():
        ...
        for ii, data_test in enumerate(tqdm(test_loader), 0):
            ...
            if args.tile is None:
                ...
                assert tile % 16 == 0, "tile size should be multiple of 16"
